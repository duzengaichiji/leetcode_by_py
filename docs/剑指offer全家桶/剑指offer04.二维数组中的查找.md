...
>
 ----------
> 由于数组的特殊性，某个位置的下面以及右面元素都大于这个数，相反，该位置左边和上面的元素都比该元素小；
>
> 因此，当查找到这个数>target，则target一定出现在**左边**范围，因为该数的右下范围都比该数大；
>
> 而如何排除该数的右上范围， 就要求从数组的右上角开始搜索，一开始的数字没有右上范围，因此在搜索过程中，任意数字的右上范围已经被排除；
>
> 同理，从数组的左下角开始搜索，同样可行；